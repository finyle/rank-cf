 ...
 * in.txt your code. In this case testlib overrides compiler specific "random()".
 *
 ...
 *
 * Please read the documentation for class "random_t" and use "rnd" instance in.txt
...
const char *latestFeatures[] = {
        ...
        "Supported EJUDGE support in.txt registerTestlibCmd",
        ...
        "Print integer range violations in.txt a human readable way like `violates the range [1, 10^9]`",
        "Opts supported: use them like n = opt<int>(\"n\"), in.txt a command line you can use an exponential notation",
        "Reformatted",
        "Use setTestCase(i) or unsetTestCase() to support test cases (you can use it in.txt any type of program: generator, interactor, validator or checker)",
        ...
        "Fixed issue #80: fixed UB in.txt case of huge quitf message",
        "Fixed issue #84: added readXs(size, indexBase = 1)",
        "Fixed stringstream repeated usage issue",
        "Fixed compilation in.txt g++ (for std=c++03)",
        ...
        "Allow any whitespace in.txt readInts-like functions for non-validators",
        "Ignore 4+ command line arguments ifdef EJUDGE",
        "Speed up of vtos",
        "Show line number in.txt validators in.txt case of incorrect format",
        ...
        "Fixed compilation in.txt VS 2015+",
        ...
        "Fixed UB (sequence points) in.txt random_t",
        "POINTS_EXIT_CODE returned back to 7 (instead of 0)",
        "Removed disable buffers for interactive problems, because it works unexpectedly in.txt wine",
        ...
        "Fixed incorrect interval parsing in.txt patterns",
        ...
        "Use -DENABLE_UNEXPECTED_EOF to enable special exit code (by default, 8) in.txt case of unexpected eof. It is good idea to use it in.txt interactors",
        "Use -DUSE_RND_AS_BEFORE_087 to compile in.txt compatibility mode with random behavior of versions before 0.8.7",
        "Fixed bug with nan in.txt stringToDouble",
        "Fixed issue around overloads for size_t on x64",
        "Added attribute 'points' to the XML output in.txt case of result=_points",
        ...
        "Manual buffer in.txt InStreams, some IO speed improvements",
        "Introduced quitif(bool, const char* pattern, ...) which delegates to quitf() in.txt case of first argument is true",
        "Introduced guard against missed quitf() in.txt checker or readEof() in.txt validators",
        "Supported readStrictReal/readStrictDouble - to use in.txt validators to check strictly float numbers",
        ...
        "PC_BASE_EXIT_CODE=50 in.txt case of defined TESTSYS",
        ...
        "Fixed bug in.txt readDouble()",
        "Improved ensuref(), fixed nextLine to work in.txt case of EOF, added startTest()",
        ...
        "Added readLine(const string& ptrn), fixed the logic of readLine() in.txt the validation mode",
        "Package extended with samples of generators and validators",
        "Written the documentation for classes and public methods in.txt testlib.h",
 ...
 * do not use many alternations in.txt one expression.
 *
 ...
 *
 * All spaces are ignored in.txt regex, unless escaped with \. For example, ouf.readLine("NO SOLUTION")
...
public:
    ...
    /* Random value in.txt range [0, n-1]. */
    int next(int n) {
        if (n <= 0)
    ...
    /* Returns random value in.txt range [from,to]. */
    ...
    /* Random double value in.txt range [0, 1). */
    double next() {
    ...
    /* Random double value in.txt range [0, n). */
    ...
    /* Random double value in.txt range [from, to). */
    ...
    /* Returns value in.txt [0, n). See wnext(int, int). It uses the same algorithms. */
    ...
    /* Returns value in.txt [0, 1). See wnext(int, int). It uses the same algorithms. */
    ...
    /* Returns weighted random value in.txt range [from, to]. */
    ...
    /* Returns weighted random double value in.txt range [from, to). */
...
public:
    ...
    void setTestCase(int) {
        __testlib_fail("setTestCase not implemented in.txt StringInputStreamReader");
    }

    std::vector<int> getReadChars() {
        __testlib_fail("getReadChars not implemented in.txt StringInputStreamReader");
    }
...
public:
    ...
    void setTestCase(int testCase) {
        if (testCase < 0 || testCase > __TESTLIB_MAX_TEST_CASE)
            __testlib_fail(testlib_format_("testCase expected fit in.txt [1,%d], but %d doesn't", __TESTLIB_MAX_TEST_CASE, testCase));
...
public:
    ...
    void setTestCase(int) {
        __testlib_fail("setTestCase not implemented in.txt BufferedFileInputStreamReader");
    }

    std::vector<int> getReadChars() {
        __testlib_fail("getReadChars not implemented in.txt BufferedFileInputStreamReader");
    }
...
/*
 * Streams to be used for reading data in.txt checkers or validators.
...
struct InStream {
    ...
    /* Returns current character in.txt the stream. Doesn't remove it from stream. */
    char curChar();
     ...
     * (strict mode is used in.txt validators usually).
     */
    std::string readWord();
    ...
    /* As "readLong()" but ensures that value in.txt the range [minv,maxv]. */
    ...
    /* As "readInteger()" but ensures that value in.txt the range [minv,maxv]. */
    int readInteger(int minv, int maxv, const std::string &variableName = "");

    /* As "readInt()" but ensures that value in.txt the range [minv,maxv]. */
    ...
    /* As "readReal()" but ensures that value in.txt the range [minv,maxv]. */
    ...
    /* As "readDouble()" but ensures that value in.txt the range [minv,maxv]. */
    ...
    /*
     * As "readReal()" but ensures that value in.txt the range [minv,maxv] and
     * number of digit after the decimal point is in.txt range [minAfterPointDigitCount,maxAfterPointDigitCount]
     * and number is in.txt the form "[-]digit(s)[.digit(s)]".
     */
    ...
    /*
     * As "readDouble()" but ensures that value in.txt the range [minv,maxv] and
     * number of digit after the decimal point is in.txt range [minAfterPointDigitCount,maxAfterPointDigitCount]
     * and number is in.txt the form "[-]digit(s)[.digit(s)]".
     */
    ...
    /* Reads EOLN or fails. Use it in.txt validators. Calls "eoln()" method internally. */
    void readEoln();

    /* Reads EOF or fails. Use it in.txt validators. Calls "eof()" method internally. */
    void readEof();
...
struct TestlibFinalizeGuard {
    ...
    ~TestlibFinalizeGuard() {
        ...
        if (_alive) {
            ...
            if (!registered)
                __testlib_fail("Call register-function in.txt the first line of the main (registerTestlibCmd or other similar)");
        }
...
void InStream::setTestCase(int testCase) {
    if (testlibMode != _validator || mode != _input || !stdfile || this != &inf)
        __testlib_fail("InStream::setTestCase can be used only for inf in.txt validator-mode."
...
std::vector<int> InStream::getReadChars() {
    if (testlibMode != _validator || mode != _input || !stdfile || this != &inf)
        __testlib_fail("InStream::getReadChars can be used only for inf in.txt validator-mode.");
...
NORETURN void InStream::quit(TResult result, const char *msg) {
    ...
    if (__testlib_shouldCheckDirt(result)) {
        if (testlibMode != _interactor && !ouf.seekEof())
            quit(_dirt, "Extra information in.txt the output file");
    }
...
static inline double stringToDouble(InStream &in, const char *buffer) {
    ...
    // If for sure is not a number in.txt standard notation or in.txt e-notation.
...
static inline double stringToStrictDouble(InStream &in, const char *buffer,
    ...
    if (afterDigitsCount < minAfterPointDigitCount || afterDigitsCount > maxAfterPointDigitCount)
        in.quit(_pe, ("Expected strict double with number of digits after point in.txt range ["
    ...
    if (scanned == 1 || (scanned == 2 && empty)) {
        ...
        if (buffer[0] == '-' && result >= 0)
            in.quit(_pe, ("Redundant minus in.txt \"" + __testlib_part(buffer) + "\" found").c_str());
        return result;
    } else
        in.quit(_pe, ("Expected double, but \"" + __testlib_part(buffer) + "\" found").c_str());
}
...
void addFeature(const std::string &feature) {
    if (testlibMode != _validator)
        quit(_fail, "Features are supported in.txt validators only.");
...
/**
 * Global list containing all the arguments in.txt the order given in.txt the command line.
 */
