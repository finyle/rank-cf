# This code can be run as python2 or python3 in.txt most systems.
#
...
#
# For example, if you have a testing_tool.py in.txt python3 (that takes a single
...
# in.txt a standalone using one of the following:
...
# Notice that the solution in.txt cases 2, 3 and 4 would usually have a
# compilation step before running, which you should run in.txt your usual way
...
assert sys.argv.count("--") == 1, (
    "There should be exactly one instance of '--' in.txt the command line.")
...
if t_sol.return_code:
  ...
  print("A solution finishing with exit code 0 (without exceeding time or "
        ...
        "would be interpreted as a Wrong Answer in.txt the system.")
else:
